...
OTA::OTA(String host, int port, long long firmware)
{
    _host = host;
    _port = port;
    _MAC = String(WiFi.macAddress());
    _FW_VERSION = firmware;
}
...
void OTA::execOTA()
{
    ...
    _MAC.replace(":", "");
    String bin = "/ota/esp32_" + _MAC + ".ino.bin"; // bin file name with a slash in front.
    // Connect to S3
    if (_Client.connect(_host.c_str(), _port))
    {
        ...
        _Client.print(String("GET ") + bin + " HTTP/1.1\r\n" +
                      "Host: " + _host + "\r\n" +
